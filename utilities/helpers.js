...
const teamGenerator = (arr) => {
  ...
  let teamOne = {},
    teamTwo = {};
  teamOne.name = `${tempAdjArr.splice(
    Math.floor(Math.random() * tempAdjArr.length),
    1
  )} ${tempNounArr.splice(Math.floor(Math.random() * tempNounArr.length), 1)}`;
  teamTwo.name = `${tempAdjArr.splice(
    Math.floor(Math.random() * tempAdjArr.length),
    1
  )} ${tempNounArr.splice(Math.floor(Math.random() * tempNounArr.length), 1)}`;
  shuffle(arr);
